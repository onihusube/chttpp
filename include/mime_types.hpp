...
#define define_semi_subtype_rhs(name, lhs) inline constexpr semi_subtype_rhs<sizeof(#name), std::remove_cvref_t<decltype(lhs)>> name = {#name}
  
  define_semi_subtype_lhs(svg, discrete_types::image);
  define_semi_subtype_rhs(xml, image/svg);
